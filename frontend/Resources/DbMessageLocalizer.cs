...
namespace WEB.APP.Localization
{
    public class DbMessageLocalizer : IMessageLocalizer
    {
        ...
        public async Task LoadResourceMessagesAsync()
        {
            ...
            //HttpResponseMessage response = await client.GetAsync(SetupHttpClient.authApiBase + "/api/auth/messystem/getmessages");
            using var client = _httpClientFactory.CreateClient("WithMicroservicesHandler");

            var response = await client.GetAsync("/api/auth/system/getmessages");
